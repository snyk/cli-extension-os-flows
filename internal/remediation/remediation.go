...
func FixesToRemediationSummary(findings Findings) (Summary, error) {
	...
	unresolved, err := calculateUnresolved(findings)
	if err != nil {
		return Summary{}, err
	}
	return Summary{
...
func calculateUnresolved(findings Findings) (unresolved []VulnerabilityInPackage, err error) {
	for finding, _ := range findings.WithUnresolvedFixes() {
...
func calculateUpgrades(findings Findings) (upgrades []Upgrade, unresolved []VulnerabilityInPackage, err error) {
	...
	for finding, fix := range findings.WithUpgradeFixes() {
		...
		for i := range upgradablePaths.upgradePath {
			...
			key := fmt.Sprintf("%s@%s", from.Name, from.Version)
