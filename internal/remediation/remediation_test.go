...
func Test_FixesToRemediationSummary(t *testing.T) {
	t.Run("no findings returns empty summary", func(t *testing.T) {
		summary, _ := remediation.FixesToRemediationSummary([]remediation.Finding{})
	...
	t.Run("pins", func(t *testing.T) {
		...
		t.Run("fully resolved (only possible scenario for pins today)", func(t *testing.T) {
			t.Run("single pin for single package returns valid summary", func(t *testing.T) {
				summary, err := remediation.FixesToRemediationSummary([]remediation.Finding{
					{
			...
			summary, err := remediation.FixesToRemediationSummary([]remediation.Finding{
				{
					...
					Fix: remediation.NewUpgradeFix(remediation.FullyResolved, remediation.UpgradeAction{
						...
						UpgradePaths: []remediation.DependencyPath{
							newDependencyPath("direct@1.2.3", "vulnerable@1.0.1"),
						},
					}),
				},
			})
			...
			summary, err := remediation.FixesToRemediationSummary([]remediation.Finding{
				{
					...
					Fix: remediation.NewUpgradeFix(remediation.FullyResolved, remediation.UpgradeAction{
						PackageName: "vulnerable",
						UpgradePaths: []remediation.DependencyPath{
							newDependencyPath("direct-1@1.2.3", "vulnerable@1.0.1"),
							newDependencyPath("direct-2@1.0.5", "vulnerable@1.0.1"),
						},
					}),
				},
			})
			...
			summary, err := remediation.FixesToRemediationSummary([]remediation.Finding{
				{
					...
					Fix: remediation.NewUpgradeFix(remediation.FullyResolved, remediation.UpgradeAction{
						PackageName: "vulnerable",
						UpgradePaths: []remediation.DependencyPath{
							newDependencyPath("direct-2@1.0.5", "transitive-1@1.0.4", "vulnerable@1.0.1"),
							newDependencyPath("direct-1@1.2.3", "vulnerable@1.0.1"),
						},
					}),
				},
			...
			summary, err := remediation.FixesToRemediationSummary([]remediation.Finding{
				{
					...
					Fix: remediation.NewUpgradeFix(remediation.FullyResolved, remediation.UpgradeAction{
						...
						UpgradePaths: []remediation.DependencyPath{
							newDependencyPath("direct@1.2.3", "vulnerable-1@1.0.1"),
						},
					}),
				},
				{
					...
					Fix: remediation.NewUpgradeFix(remediation.FullyResolved, remediation.UpgradeAction{
						...
						UpgradePaths: []remediation.DependencyPath{
							newDependencyPath("direct@1.4.0", "vulnerable-2@1.0.1"),
						},
					}),
				},
			})
			...
			summary, err := remediation.FixesToRemediationSummary([]remediation.Finding{
				{
					...
					Fix: remediation.NewUpgradeFix(remediation.FullyResolved, remediation.UpgradeAction{
						PackageName: "vulnerable-1",
						UpgradePaths: []remediation.DependencyPath{
							newDependencyPath("direct-1@1.2.3", "vulnerable-1@1.0.1"),
						},
					}),
				},
				{
					...
					Fix: remediation.NewUpgradeFix(remediation.FullyResolved, remediation.UpgradeAction{
						PackageName: "vulnerable-2",
						UpgradePaths: []remediation.DependencyPath{
							newDependencyPath("direct-2@1.4.0", "vulnerable-2@1.0.1"),
						},
					}),
				},
			})
...
func equalSummaries(t *testing.T, expected remediation.Summary, actual remediation.Summary) {
	t.Helper()
...
func equalUpgrades(t *testing.T, expected []remediation.Upgrade, actual []remediation.Upgrade) {
	t.Helper()
...
func equalVulnerabilityInPackages(t *testing.T, expected []remediation.VulnerabilityInPackage, actual []remediation.VulnerabilityInPackage) {
	require.Equal(t, len(expected), len(actual))
...
func vulnInPackageComparator(a remediation.VulnerabilityInPackage, b remediation.VulnerabilityInPackage) int {
...
func newPackage(p string) remediation.Package {
	sp := strings.Split(p, "@")
	return remediation.Package{sp[0], sp[1]}
}
